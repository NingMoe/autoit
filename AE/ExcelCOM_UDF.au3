...
; Syntax:           _ExcelInsert($oExcel, $iRangeOrRow, $iColumn = 1, $iShiftDirection)
; Parameter(s):     $oExcel - An Excel object opened by a preceding call to _ExcelBookOpen() or _ExcelBookNew()
...
;xlDiagonalDown, xlDiagonalUp, xlEdgeBottom, xlEdgeLeft, xlEdgeRight, xlEdgeTop, xlInsideHorizontal, or xlInsideVertical.
;$xlThick							= 4
;$xlThin							= 2
;$xlContinuous						= 1
#cs
;Func _ExcelCreateBorders($oExcel, $iTop, $iBottom, $iLeft, $iRight, $iInVert, $iInHoriz, $iLineStyle, $sRangeOrRow, $iColumn = 1)
	If NOT IsObj($oExcel) Then Return SetError(1, 0, 0)
	If NOT StringRegExp($sRangeOrRowStart, "[A-Z,a-z]", 0) Then
		
	Else
		$oExcel.Range($sRange).Select
	EndIf
...
;
;===============================================================================
Func _ExcelFindInRange($oExcel, $sFindWhat, $sRangeOrRowStart, $iColStart = 1, $iRowEnd = 1, $iColEnd = 1, $iDataType = 0, $iWholeOrPart = 2, $fMatchCase = False, $fMatchFormat = False)
...
;
;===============================================================================
Func _ExcelReplaceInRange($oExcel, $sFindWhat, $sReplaceWith, $sRangeOrRowStart, $iColStart = 1, $iRowEnd = 1, $iColEnd = 1, $iWholeOrPart = 2, $fMatchCase = False, $fMatchFormat = False, $fReplaceFormat = False)
	If NOT IsObj($oExcel) Then Return SetError(1, 0, 0)
...
To be written:
	...
	_ExcelPrintRange()
	_ExcelPrintSetup()
	_ExcelBookProperties()


----------------Excel PrintOut() method
range.PrintOut([From], [To], [Copies], [Preview], [ActivePrinter], [PrintToFile], [Collate], [PrToFileName])  
--------------------------------------------------------------------------------
Prints the specified range.
 
From - The number of the first page to print.
To - The number of the last page to print.
Copies - The number of copies to print.
Preview - True to display Print Preview.
ActivePrinter - The name of the active printer.
PrintToFile - True prints to a file.
Collate - True to collate multiple copies.
PrToFileName - If PrintToFile is True, specifies the name of a file to print to.
#ce
...
Filename - The name of the file to save. Default is current filename.
 
FileFormat - One of the following xlFileFormat settings: xlCS, xlCSVMSDOS, xlCurrentPlatformText, xlDBF3, xlDIF,
xlExcel2FarEast, xlExcel4, xlAddIn, xlCSVMac, xlCSVWindows, xlDBF2, xlDBF4, xlExcel2, xlExcel3, xlExcel4Workbook,
xlExcel5, xlExcel7, xlExcel9795, xlHtml, xlIntlAddIn, xlIntlMacro, xlSYLK, xlTemplate, xlTextMac, xlTextMSDOS,
xlTextPrinter, xlTextWindows, xlUnicodeText, xlWebArchive, xlWJ2WD1, xlWJ3, xlWJ3FJ3, xlWK1, xlWK1ALL, xlWK1FMT,
xlWK3, xlWK3FM3, xlWK4, xlWKS, xlWorkbookNormal, xlWorks2FarEast, xlWQ1, xlXMLSpreadsheet.
 
Password - A password the user must enter to open the file.
 
WriteResPassword - A password the user must enter to open the file for read/write access.
 
ReadOnlyRecommended - True causes Excel to display a dialog box recommending the file be opened for read-only
access when the user opens the file; False does not display the dialog box. Default is False.
 
CreateBackup - True creates a backup version of the file when the file is saved; False does not create a backup.
Default is False.
 
AccessMode - One of the following xlSaveAccessMode settings: xlExclusive, xlNoChange (default), xlShared. Use
xlShared to share a workbook, xlExclusive to remove sharing.
 
ConflictResolution - For shared workbooks, one of the following xlSaveConflictResolution settings:
xlUserResolution (default), xlLocalSessionChanges, xlOtherSessionChanges.
 
AddToMru - True adds this workbook to Excel's most recently used file list on the File menu; False omits this
workbook from the list. Default is False.
 
TextCodepage - For foreign-language versions of Excel only, the code page to save the workbook with.
 
TextVisualLayout - For foreign-language versions of Excel only, orientation to use when presenting data.
 
Local - True saves the workbook against the local user language; False saves the workbook against the language
used in Visual Basic (typically, English).
