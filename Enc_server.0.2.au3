...
Func Received($iError, $sReceived); And we also registered this! Our homemade do-it-yourself function gets called when something is received.
	...
	DIM $year=@YEAR
	 ...
	 FileWriteLine($writefile,@hour&":"&@MIN&":"&@SEC&" , "&$ip&", "&$sReceived )
	 FileClose($writefile)
	 ...
	 ;tooltip("")
		...
		Select
			case  $dbrecord[1]=0  ; This is to select key value and then send to users.
					Local $hQuery
					local $aRow
					local $sMsg
					_SQlite_Query (-1, "SELECT STRING FROM users where NAME='"&$dbrecord[2]&"';", $hQuery) ; the query
					While _SQLite_FetchData ($hQuery, $aRow) = $SQLITE_OK
						$sMsg &= $aRow[0]
					WEnd
					;MsgBox(0,"String",$sMsg&" <> "&$dbrecord[4])
					if $sMsg=$dbrecord[4] then 
		...
		Case	$dbrecord[1]=1  ; This is for insert value to DB
		;_SQLite_Exec(-1,"Create table Encryption ( KEY ,NAME ,MD5 ,INTIME);" )
		...
		Case $dbrecord[1]=2 ; This is for insert New user string
			MsgBox(0,"New User",$dbrecord[2]&" < > "&$dbrecord[4],5)
			local $x
			$x=_newuser_str($dbrecord[2],$dbrecord[4])
			if $x=1 then
				_TCP_Server_Send( "Done" )
				sleep(500)
				Disconnect(1)
			Else
				_TCP_Server_Send( "Error" )
				ConsoleWrite(@CRLF&"_newuser_str error")
				Disconnect(1)
			EndIf
		case Else
			_TCP_Server_Send( "Error" )
